...
function KindPicker({ value, onChange }) {
  return (
    <div style={{ display: "flex", gap: 6, background: "var(--cream)", padding: 4, borderRadius: 12, marginBottom: 2 }}>
      {[
    ...
    <div className="cdv-split" style={{ height: "100%", display: "grid", gridTemplateColumns: "1fr 380px", minHeight: 0 }}>
              ...
              {sorted.map((e) => {
                ...
                <div key={e.id} style={{ display: "flex", alignItems: "center", gap: 14, background: "#fff", border: "1px solid " + (entrada ? "oklch(0.88 0.06 150)" : "var(--line)"), borderRadius: "var(--r)", padding: "14px 16px" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 700, fontSize: 15, color: "var(--ink)" }}>{e.concept}</div>
                  ...
                  <div style={{ fontFamily: "var(--display)", fontWeight: 700, fontSize: 17, color: entrada ? VERDE : "var(--navy)" }}>
      ...
      <div style={{ background: "#fff", display: "flex", flexDirection: "column", minHeight: 0 }}>
        <div style={{ padding: "24px 26px", flex: 1, overflowY: "auto" }}>
          <div style={{ fontWeight: 700, fontSize: 13.5, color: "var(--muted)", textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 14 }}>Nuevo movimiento</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div style={{ position: "relative" }}>
              <span style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: "var(--muted)", fontFamily: "var(--display)", fontWeight: 700, fontSize: 18 }}>Q</span>
              <input
                value={amount}
                onChange={(e) => setAmount(e.target.value.replace(/[^0-9.]/g, ""))}
                inputMode="decimal"
                placeholder="0.00"
                style={{ width: "100%", padding: "12px 14px 12px 34px", border: "1px solid var(--line)", borderRadius: 12, fontFamily: "var(--display)", fontSize: 18, fontWeight: 700, color: "var(--navy)", outline: "none", boxSizing: "border-box" }}
              />
        ...
        <div style={{ padding: "18px 26px 24px", borderTop: "1px solid var(--line)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14, color: "var(--muted)", padding: "4px 0" }}>
            ...
            <span style={{ fontWeight: 700, color: "var(--ink)" }}>{money(totalCash)}</span>
          </div>
          {totalEntradas > 0 && (
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14, color: "var(--muted)", padding: "4px 0" }}>
              <span>Entradas de dinero</span>
              <span style={{ fontWeight: 700, color: VERDE }}>+{money(totalEntradas)}</span>
            </div>
          )}
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginTop: 6 }}>
            <span style={{ fontWeight: 700, fontSize: 16, color: "var(--navy)" }}>Gastos del turno</span>
